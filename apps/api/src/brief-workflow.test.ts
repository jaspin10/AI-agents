...
function fixture(role='owner'){
 ...
 const store:StudioStore={revision:async()=>null,lineage:async()=>[],confirmLineage:async()=>{throw new Error("unused");},get:async(k,id)=>{const r=records.get(id);return r?.kind===k?structuredClone(r):null;},list:async k=>[...records.values()].filter(r=>r.kind===k),history:async()=>[],
