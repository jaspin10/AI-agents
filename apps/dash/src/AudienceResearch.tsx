...
export function AudienceResearch(){
 ...
 function load(){getJson<{records:ResearchRow[];topics:Topic[]}>('/api/studio/research').then(p=>{setRows(p.records);setTopics(p.topics);}).catch(e=>setMessage(e.message));}useEffect(load,[]);
 ...
 return <details className="card studio"><summary>Audience questions & topic opportunities</summary><p className="dim">Enter anonymous, approved text only. No private messages, contact details or inferred sensitive traits. Public references are for studying structure, not copying scripts.</p>
