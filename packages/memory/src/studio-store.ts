...
export function createStudioStore():StudioStore {
 ...
 return {
  ...
  async save(v){const {data,error}=await db.rpc('save_studio_record',{p_id:v.id,p_kind:v.kind,p_entity:v.entityKey,p_expected:v.expectedVersion,p_body:v.body,p_actor:v.actor,p_request:v.requestId});if(error)throw new Error(error.message.includes('version_conflict')?'version_conflict':error.message.includes('request_conflict')?'request_conflict':'studio_unavailable');return parse(data as Record<string,unknown>);},
 };
}
