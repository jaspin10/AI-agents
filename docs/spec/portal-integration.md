...
3. (~30 min, portal repo) The tab itself — replace the Analytics placeholder body so it opens the hosted analyst dash URL in a new browser tab (chosen over iframe: no CSP/embedding headaches, honest about being a separate app). Needs the Step 4 URL to exist first.
