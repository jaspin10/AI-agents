...
- **Bug found 2026-09-10 — every live save has been failing 500 since ship:** `PUT /api/analysis/:contentId` writes `content.hypothesis` via the mechanical tag format, but migration 0001's `content_hypothesis_check` constraint (`hypothesis in ('H1','H2','H3')`, left over from the pre-X1 taxonomy) was never dropped when 0005 replaced that taxonomy. Every save hits `Supabase content set tags failed: ... violates check constraint "content_hypothesis_check"`. Hit by both Jas (`548b7c87-...`) and Eknoor (`8af71ff0-...`) on their first live save attempts, confirmed from Railway `analyst-dash` deploy logs. **Fix drafted:** migration `0010_drop_content_hypothesis_check.sql` drops the stale constraint — same "no CHECK enums" principle 0005 already applied everywhere else. Branch `fix/content-hypothesis-check`, PR open against `milestone-2`. **Not yet applied to the analyst Supabase project** — this session's Supabase MCP is scoped to the portal project only, so 0010 needs to be run manually (or from a session with the analyst project connected) before a save will succeed. Still to verify live once it's applied: Eknoor tagging one actual video end to end (save → confirm it appears correctly, including a ref and an ad run).
...
**Ordering with X-B2 reversed 2026-09-10 (Jas).** X-B2 no longer blocks X3 — it is the other way round. Reconciling enrollment numbers in the abstract means guessing at a gap; it is far easier once a concrete view shows which months are short and by how much. So **X3 builds the view first, and the view is what makes X-B2 doable.**
